...
/* LED theo pitch/roll */
void indicate_by_pitch_roll(float pitch_deg, float roll_deg)
{
    ...
    if (toggle)
    {
        if (pitch_deg > 3.0f)       GPIOD_ODR |= (1<<13); // trước
        else if (pitch_deg < -3.0f) GPIOD_ODR |= (1<<15); // sau
        else if (roll_deg  > 3.0f)  GPIOD_ODR |= (1<<12); // phải
        else if (roll_deg  < -3.0f) GPIOD_ODR |= (1<<14); // trái
    }
}
